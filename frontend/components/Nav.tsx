  ...
  return (
    <nav className={`glass-nav fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${scrolled ? 'py-0 border-b border-[#F0D9DC] shadow-sm' : 'py-1'}`}>
      <div className={`max-w-[1280px] mx-auto px-4 sm:px-6 flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-14' : 'h-16'}`}>
        ...
        <button className="md:hidden p-3 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg text-[#6B6B6B]" onClick={() => setOpen(!open)}>
      ...
      {open && (
        <>
          <div className="md:hidden fixed inset-0 top-full bg-black/20 backdrop-blur-sm z-40 transition-opacity" onClick={() => setOpen(false)} />
          <div className="md:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-md border-b border-[#F0D9DC] px-4 py-5 flex flex-col gap-3 shadow-xl z-50 max-h-[calc(100vh-4rem)] overflow-y-auto">
            <Link to="/live-board" onClick={() => setOpen(false)}
              className="text-sm font-medium text-[#171717] py-3 border-b border-[#E8E8E8]"
              style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              Live Board
            </Link>
            ...
            {user && (
              <div className="py-3 border-b border-[#E8E8E8] flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-[#C1121F] text-white flex items-center justify-center font-bold text-sm">
                  {(user.displayName || user.email?.split('@')[0] || 'U')[0].toUpperCase()}
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <Link to="/donor/register" onClick={() => setOpen(false)} className="btn-secondary w-full justify-center py-3 min-h-[44px] text-sm">
                Become a Donor
              </Link>
              <Link to="/request/new" onClick={() => setOpen(false)} className="btn-primary w-full justify-center py-3 min-h-[44px] text-sm">
                Request Blood
              </Link>
            </div>
            ...
            {user ? (
              <div className="flex flex-col mt-1">
                <Link to="/dashboard" onClick={() => setOpen(false)} className="btn-secondary w-full justify-center py-3 min-h-[44px] text-sm mb-3">
                  Dashboard
                </Link>
                <button 
                  onClick={handleLogoutClick}
                  className="text-sm font-medium text-[#C1121F] py-3 text-left w-full min-h-[44px]"
                >
                  Logout
                </button>
              </div>
            ) : (
              <div className="flex flex-col sm:flex-row gap-3 pt-2 border-t border-[#E8E8E8] mt-2">
                <Link to="/login" onClick={() => setOpen(false)} className="btn-ghost w-full justify-center py-3 min-h-[44px] text-sm text-center border border-transparent hover:border-[#F0D9DC]">
                  Login
                </Link>
                <Link to="/signup" onClick={() => setOpen(false)} className="btn-secondary w-full justify-center py-3 min-h-[44px] text-sm text-center">
                  Sign Up
                </Link>
              </div>
            )}
